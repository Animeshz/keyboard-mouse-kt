...
void Send(const Napi::CallbackInfo& info) {
    ...
    Keyboard::getInstance()->sendEvent(scanCode, isPressed);
}

Napi::Value IsPressed(const Napi::CallbackInfo& info) {
    int scanCode = info[0].As<Napi::Number>().Int32Value();

    bool res = Keyboard::getInstance()->isPressed(scanCode);
    return Napi::Boolean::New(info.Env(), res);
}

Napi::Value IsCapsLockOn(const Napi::CallbackInfo& info) {
    bool res = Keyboard::getInstance()->isCapsLockOn();
    return Napi::Boolean::New(info.Env(), res);
}

Napi::Value IsNumLockOn(const Napi::CallbackInfo& info) {
    bool res = Keyboard::getInstance()->isNumLockOn();
    return Napi::Boolean::New(info.Env(), res);
}

Napi::Value IsScrollLockOn(const Napi::CallbackInfo& info) {
    bool res = Keyboard::getInstance()->isScrollLockOn();
    return Napi::Boolean::New(info.Env(), res);
}

int Init() {
    int ret = 1;
    if (Keyboard::getInstance() != NULL) ret = 0;
...
Napi::Value StartReadingEvents(const Napi::CallbackInfo& info) {
    ...
    int res = Keyboard::getInstance()->startReadingEvents(EmitEventToTSCallback);
    return Napi::Number::New(env, res);
}

void StopReadingEvents(const Napi::CallbackInfo& info) { Keyboard::getInstance()->stopReadingEvents(); }
