...
/**
 * Receives the event from the Windows MessageQueue and passes it to [Keyboard].
...
internal fun lowLevelKeyboardProc(nCode: Int, wParam: WPARAM, lParam: LPARAM): LRESULT {
    try {
        ...
        if (vk != VK_PACKET && keyInfo.flags.toInt() and Keyboard.FAKE_ALT != Keyboard.FAKE_ALT) {
            ...
            Keyboard.process(keyEventType, vk, keyInfo.scanCode.toInt(), extended)
        }
    } catch (e: Exception) {
        e.printStackTrace()
    }
